...
func Usage() {
  ...
  fmt.Fprintln(os.Stderr, "   GetTagsByUID(Intreq i)")
  fmt.Fprintln(os.Stderr)
  os.Exit(0)
}
...
func main() {
  ...
  client := rpc.NewTagServiceClient(thrift.NewTStandardClient(iprot, oprot))
  ...
  case "GetTagsByUID":
    if flag.NArg() - 1 != 1 {
      fmt.Fprintln(os.Stderr, "GetTagsByUID requires 1 args")
      flag.Usage()
    }
    arg15 := flag.Arg(1)
    ...
    value0 := argvalue0
    fmt.Print(client.GetTagsByUID(context.Background(), value0))
    fmt.Print("\n")
    break
  case "":
    Usage()
